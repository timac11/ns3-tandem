...
protected:
  ...
  virtual void TxPacketOk (Ptr<const Packet> item);
   /**
   * packets was requested channel access
   *
   * \param hdr the header of the packet that we successfully sent
   */
  virtual void RequestChannelAccess (const WifiMacQueueItem &item);
...
private:
  ...
  TracedCallback<Ptr<const Packet>> m_txOkPacketCallback; ///< transmit OK callback
  TracedCallback<const WifiMacQueueItem &> m_requestChannelAccessCallback; ///< transmit OK callback
