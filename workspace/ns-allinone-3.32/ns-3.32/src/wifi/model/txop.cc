...
void
Txop::SetTxOkPacketCallback (TxOkPacket callback)
{
  NS_LOG_FUNCTION (this << &callback);
  m_txOkPacketCallback = callback;
}

void
Txop::SetRequestAccessCallback(RequestAccess callback)
{
  NS_LOG_FUNCTION (this << &callback);
...
Txop::NotifyAccessRequested (void)
{
  ...
  // verification for empty is not empty
  if (!m_queue->IsEmpty()) {
    Ptr<const WifiMacQueueItem> nextPacket = m_queue->Peek();
    if (!m_requestAccessCallback.IsNull () && nextPacket != NULL)
        {
          m_requestAccessCallback (*nextPacket);
        }
  }
}
...
Txop::GotAck (void)
{
  ...
  if (!NeedFragmentation ()
      || IsLastFragment ())
    {
      ...
      if (!m_txOkPacketCallback.IsNull () && m_currentPacket != NULL)
        {
          m_txOkPacketCallback (m_currentPacket);
        } 
